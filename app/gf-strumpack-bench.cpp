...
int main(int argc, char** argv) {
    ...
    argparse::ArgumentParser program("gf-strumpack-bench");
    ...
    program.add_argument("config").help("Configuration file (.toml) — same as tandem, "
                                        "mode must be QDGreen, [hmatrix] section sets tolerances.");
    ...
    if (cfg->mode != SeasMode::QuasiDynamicDiscreteGreen) {
        std::cerr << "gf-strumpack-bench: mode must be QDGreen" << std::endl;
        return -1;
    }
    ...
    if (cfg->mesh_file) {
        ...
        if (rank == 0) {
            ...
            if (!ok) std::cerr << *cfg->mesh_file << std::endl << parser.getErrorMessage();
        }
        MPI_Bcast(&ok, 1, MPI_CXX_BOOL, 0, PETSC_COMM_WORLD);
        if (!ok) { PetscFinalize(); return -1; }
    ...
    bool do_butterfly = program.get<bool>("--butterfly");
    double rel_err = validateGFStrumpack(*mesh, *cfg, do_butterfly);

    PetscErrorCode ierr = PetscFinalize();
    if (rel_err >= 0.0 && rel_err >= cfg->hmatrix_config.rtol) return 1;
