...
PetscErrorCode RandEigsMax(Mat A, PetscInt k, PetscInt o, PetscInt pits, PetscRandom prand,
    ...
    CHKERRQ(ierr);
    if (M != N)
        SETERRQ2(comm, PETSC_ERR_SUP, "Only valid for square matrices, found M = %D, N = %D\n", M,
                 N);
    if (k + o > M)
        SETERRQ4(comm, PETSC_ERR_SUP,
                 "Random matrix has %D + %D = %D columns. Max num. columns is %D\n", k, o, k + o,
                 M);
