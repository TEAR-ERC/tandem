    ...
    if (prefix.has_value()) {
        ...
        if (rank == 0) {
          ...
          if (!exists) {
              ...
              if (!ret) throw std::runtime_error("Failed to create directory!");
          }
        }
...
void SeasQDDiscreteGreenOperator::write_discrete_greens_operator(LocalSimplexMesh<DomainDimension> const& mesh, PetscInt current_gf, PetscInt n_gf) {
  PetscViewer v;
...
void SeasQDDiscreteGreenOperator :: write_facet_labels_IS(LocalSimplexMesh<DomainDimension> const& mesh)
{
  ...
  CHKERRTHROW(ISCreateGeneral(PetscObjectComm((PetscObject)G_),nfacets*facet_size,(const PetscInt*)idx_,PETSC_USE_POINTER,&is));
...
void SeasQDDiscreteGreenOperator::create_permutation_redundant_IS(LocalSimplexMesh<DomainDimension> const& mesh, IS is)
{
  ...
  using map_t = std::unordered_map<Simplex<DomainDimension-1u>, std::size_t, SimplexHash<DomainDimension-1u>>;
  ...
  auto create_permutation_matrix = [this,&comm, mb_offset,&fault_map_index](PetscInt m, PetscInt M, PetscInt block_size, bool swap) {
    ...
    for (std::size_t bndNo = 0; bndNo < this->adapter().fault_map().local_size(); ++bndNo) {
      ...
      for (PetscInt d=0; d<block_size; d++) {
        CHKERRTHROW(MatSetValue(perm,block_size * from + d,block_size * to + d,1.0,INSERT_VALUES));
      }
    }
...
PetscInt SeasQDDiscreteGreenOperator::load_discrete_greens_operator(LocalSimplexMesh<DomainDimension> const& mesh, PetscInt n_gf) {
  PetscViewer v;
  PetscLogDouble t0,t1;
  MPI_Comm comm = base::comm();
  int commsize;
...
void SeasQDDiscreteGreenOperator::partial_assemble_discrete_greens_function(LocalSimplexMesh<DomainDimension> const& mesh, PetscInt current_gf, PetscInt n_gf) {
  auto slip_block_size = base::friction().slip_block_size();
