...
static PetscErrorCode _TSAdaptMembersView(TSAdapt adapt, PetscViewer viewer) {
    ...
    if (!viewer) {
        ierr = PetscViewerASCIIGetStdout(PetscObjectComm((PetscObject)adapt), &viewer);
        CHKERRQ(ierr);
    }
    ...
    if (!isbinary)
        SETERRQ(PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG,
                "Invalid viewer; open viewer with PetscViewerBinaryOpen()");

    /* Missing content which should be in TSAdaptView()*/
    ierr = PetscViewerBinaryWrite(viewer, &adapt->always_accept, 1, PETSC_BOOL);
    CHKERRQ(ierr);
    ierr = PetscViewerBinaryWrite(viewer, &adapt->safety, 1, PETSC_DOUBLE);
    CHKERRQ(ierr);
    ierr = PetscViewerBinaryWrite(viewer, &adapt->reject_safety, 1, PETSC_DOUBLE);
    CHKERRQ(ierr);
    ierr = PetscViewerBinaryWrite(viewer, adapt->clip, 2, PETSC_DOUBLE);
    CHKERRQ(ierr);
    ierr = PetscViewerBinaryWrite(viewer, &adapt->dt_min, 1, PETSC_DOUBLE);
    CHKERRQ(ierr);
    ierr = PetscViewerBinaryWrite(viewer, &adapt->dt_max, 1, PETSC_DOUBLE);
    CHKERRQ(ierr);
    ierr = PetscViewerBinaryWrite(viewer, &adapt->ignore_max, 1, PETSC_DOUBLE);
    CHKERRQ(ierr);
    ierr = PetscViewerBinaryWrite(viewer, &adapt->glee_use_local, 1, PETSC_BOOL);
    CHKERRQ(ierr);
    ierr = PetscViewerBinaryWrite(viewer, &adapt->scale_solve_failed, 1, PETSC_DOUBLE);
    CHKERRQ(ierr);
    ierr = PetscViewerBinaryWrite(viewer, adapt->matchstepfac, 2, PETSC_DOUBLE);
    CHKERRQ(ierr);
    ierr = PetscViewerBinaryWrite(viewer, &adapt->timestepjustdecreased_delay, 1, PETSC_INT);
    CHKERRQ(ierr);
...
static PetscErrorCode _TSView(TS ts, PetscViewer viewer) {
    ...
    if (!viewer) {
        ierr = PetscViewerASCIIGetStdout(PetscObjectComm((PetscObject)ts), &viewer);
        CHKERRQ(ierr);
    }
    PetscValidHeaderSpecific(viewer, PETSC_VIEWER_CLASSID, 2);
    ...
    CHKERRQ(ierr);
    ierr = PetscObjectTypeCompare((PetscObject)viewer, PETSCVIEWERSTRING, &isstring);
    CHKERRQ(ierr);
    ierr = PetscObjectTypeCompare((PetscObject)viewer, PETSCVIEWERBINARY, &isbinary);
    CHKERRQ(ierr);
    ...
    if (iascii) {
        ...
        CHKERRQ(ierr);
        if (!rank) {
            ierr = PetscViewerBinaryWrite(viewer, &classid, 1, PETSC_INT);
            CHKERRQ(ierr);
            ierr = PetscStrncpy(type, ((PetscObject)ts)->type_name, 256);
            CHKERRQ(ierr);
            ierr = PetscViewerBinaryWrite(viewer, type, 256, PETSC_CHAR);
            CHKERRQ(ierr);

            /* PETSC FIX START */
            /* missing content */
            ierr = PetscViewerBinaryWrite(viewer, &ts->steps, 1, PETSC_INT);
            CHKERRQ(ierr);
            ierr = PetscViewerBinaryWrite(viewer, &ts->ptime, 1, PETSC_DOUBLE);
            CHKERRQ(ierr);
            ierr = PetscViewerBinaryWrite(viewer, &ts->time_step, 1, PETSC_DOUBLE);
            CHKERRQ(ierr);
            ierr = PetscViewerBinaryWrite(viewer, &ts->ptime_prev, 1, PETSC_DOUBLE);
            CHKERRQ(ierr);
            ierr = PetscViewerBinaryWrite(viewer, &ts->ptime_prev_rollback, 1, PETSC_DOUBLE);
            CHKERRQ(ierr);
            ierr = PetscViewerBinaryWrite(viewer, &ts->solvetime, 1, PETSC_DOUBLE);
            CHKERRQ(ierr);
            /* PETSC FIX END */
        }
        if (ts->ops->view) {
            ierr = (*ts->ops->view)(ts, viewer);
            CHKERRQ(ierr);
        }
        /* PETSC BUG */
        ...
        /* PETSC FIX START */
        if (!ts->adapt) {
            ierr = TSAdaptCreate(PetscObjectComm((PetscObject)ts), &ts->adapt);
            CHKERRQ(ierr);
            ierr = TSAdaptSetType(ts->adapt, TSADAPTNONE);
            CHKERRQ(ierr);
        }
        ierr = TSAdaptView(ts->adapt, viewer);
        CHKERRQ(ierr);

        /* PETSC FIX END */
        ierr = DMView(ts->dm, viewer);
        CHKERRQ(ierr);
        ierr = VecView(ts->vec_sol, viewer);
        CHKERRQ(ierr);
        ierr = DMGetDMTS(ts->dm, &sdm);
        CHKERRQ(ierr);
        ierr = DMTSView(sdm, viewer);
        CHKERRQ(ierr);

        ierr = _TSAdaptMembersView(ts->adapt, viewer);
        CHKERRQ(ierr);
...
typedef struct {
    ...
    Vec*
        YdotRHS_fast; /* Function evaluations for the non-stiff part and contains fast components */
    Vec*
        YdotRHS_slow; /* Function evaluations for the non-stiff part and contains slow components */
    ...
    PetscInt
        slow; /* flag indicates call slow components solver (0) or fast components solver (1) */
...
static PetscErrorCode _TSImplView_RK(TS ts, PetscViewer viewer) {
    ...
    if (iascii) {
        ...
        if (same) {
            ...
            ierr = TSRKGetTableau(ts, &s, NULL, NULL, NULL, NULL, NULL, NULL, &FSAL);
            CHKERRQ(ierr);
            for (i = 0; i < s; i++) {
                /*
                        PetscBool same;
                        PetscInt NR;
                        printf("rk->YdotRHS[%d] \n",i);
                        PetscObjectTypeCompare((PetscObject)rk->YdotRHS[i],VECSEQ,&same); if (same)
                   printf("Vec seq\n");
                        PetscObjectTypeCompare((PetscObject)rk->YdotRHS[i],VECMPI,&same); if (same)
                   printf("Vec mpi\n");
                        PetscObjectTypeCompare((PetscObject)rk->YdotRHS[i],VECNEST,&same); if (same)
                   printf("Vec nest\n"); VecGetSize(rk->YdotRHS[i],&NR); printf("length %d\n",NR);
                */
                ierr = VecNestUpgradeOperations(rk->YdotRHS[i]);
                CHKERRQ(ierr);
                ierr = VecView(rk->YdotRHS[i], viewer);
                CHKERRQ(ierr);
            }
        }
    } else if (isdraw) {
...
static PetscErrorCode _TSAdaptMembersLoad(TSAdapt adapt, PetscViewer viewer) {
    ...
    ierr = PetscViewerBinaryRead(viewer, &adapt->always_accept, 1, NULL, PETSC_BOOL);
    CHKERRQ(ierr);
    ierr = PetscViewerBinaryRead(viewer, &adapt->safety, 1, NULL, PETSC_DOUBLE);
    CHKERRQ(ierr);
    ierr = PetscViewerBinaryRead(viewer, &adapt->reject_safety, 1, NULL, PETSC_DOUBLE);
    CHKERRQ(ierr);
    ierr = PetscViewerBinaryRead(viewer, adapt->clip, 2, NULL, PETSC_DOUBLE);
    CHKERRQ(ierr);
    ierr = PetscViewerBinaryRead(viewer, &adapt->dt_min, 1, NULL, PETSC_DOUBLE);
    CHKERRQ(ierr);
    ierr = PetscViewerBinaryRead(viewer, &adapt->dt_max, 1, NULL, PETSC_DOUBLE);
    CHKERRQ(ierr);
    ierr = PetscViewerBinaryRead(viewer, &adapt->ignore_max, 1, NULL, PETSC_DOUBLE);
    CHKERRQ(ierr);
    ierr = PetscViewerBinaryRead(viewer, &adapt->glee_use_local, 1, NULL, PETSC_BOOL);
    CHKERRQ(ierr);
    ierr = PetscViewerBinaryRead(viewer, &adapt->scale_solve_failed, 1, NULL, PETSC_DOUBLE);
    CHKERRQ(ierr);
    ierr = PetscViewerBinaryRead(viewer, adapt->matchstepfac, 2, NULL, PETSC_DOUBLE);
    CHKERRQ(ierr);
    ierr = PetscViewerBinaryRead(viewer, &adapt->timestepjustdecreased_delay, 1, NULL, PETSC_INT);
    CHKERRQ(ierr);
...
static PetscErrorCode _TSLoad(TS ts, PetscViewer viewer) {
    ...
    ierr = PetscViewerBinaryRead(viewer, &classid, 1, NULL, PETSC_INT);
    CHKERRQ(ierr);
    if (classid != TS_FILE_CLASSID)
        SETERRQ(PetscObjectComm((PetscObject)ts), PETSC_ERR_ARG_WRONG, "Not TS next in file");
    ierr = PetscViewerBinaryRead(viewer, type, 256, NULL, PETSC_CHAR);
    CHKERRQ(ierr);

    /* PETSC FIX START */
    /* missing content */
    ierr = PetscViewerBinaryRead(viewer, &ts->steps, 1, NULL, PETSC_INT);
    CHKERRQ(ierr);
    ierr = PetscViewerBinaryRead(viewer, &ts->ptime, 1, NULL, PETSC_DOUBLE);
    CHKERRQ(ierr);
    ierr = PetscViewerBinaryRead(viewer, &ts->time_step, 1, NULL, PETSC_DOUBLE);
    CHKERRQ(ierr);
    ierr = PetscViewerBinaryRead(viewer, &ts->ptime_prev, 1, NULL, PETSC_DOUBLE);
    CHKERRQ(ierr);
    ierr = PetscViewerBinaryRead(viewer, &ts->ptime_prev_rollback, 1, NULL, PETSC_DOUBLE);
    CHKERRQ(ierr);
    ierr = PetscViewerBinaryRead(viewer, &ts->solvetime, 1, NULL, PETSC_DOUBLE);
    CHKERRQ(ierr);
    /* PETSC BUG FIX END */

    ierr = TSSetType(ts, type);
    CHKERRQ(ierr);
    if (ts->ops->load) {
        ierr = (*ts->ops->load)(ts, viewer);
        CHKERRQ(ierr);
    }
    ...
    if (!ts->adapt) {
        /*
           The reason why we create/load here is because _TSView() is guaranteed to write out a
           TSAdapt object. However, depending on the TSType TSAdapt may not be created by default.
           Also, some implementations of TS will actually create / load TSAdapt _within_
           ts->ops->load(). Hence if we create the TSAdapt (because it didn't exist), we must also
           load it. Putting TSAdaptLoad() outside of this if statement may sometimes result in
           TSAdapt being attempted to be loaded twice.
        */
        ierr = TSAdaptCreate(PetscObjectComm((PetscObject)ts), &ts->adapt);
        CHKERRQ(ierr);
        ierr = TSAdaptLoad(ts->adapt, viewer);
        CHKERRQ(ierr);
    }
    ...
    CHKERRQ(ierr);
    ierr = DMLoad(dm, viewer);
    CHKERRQ(ierr);
    ierr = TSSetDM(ts, dm);
    CHKERRQ(ierr);
    if (!ts->vec_sol) {
        ierr = DMCreateGlobalVector(ts->dm, &ts->vec_sol);
        CHKERRQ(ierr);
    }
    ierr = VecLoad(ts->vec_sol, viewer);
    CHKERRQ(ierr);
    ierr = DMGetDMTS(ts->dm, &sdm);
    CHKERRQ(ierr);
...
PetscErrorCode _TSImplLoad_RK(TS ts, PetscViewer viewer) {
    ...
    if (same) {
        ...
        for (i = 0; i < s; i++) {
            ...
            if (rk->YdotRHS[i]) {
                ierr = VecLoad(rk->YdotRHS[i], viewer);
                CHKERRQ(ierr);
            }
        }
    }
...
typedef enum {
    TSCP_STORAGE_NONE = 0,
    TSCP_STORAGE_UNLIMITED,
    TSCP_STORAGE_LIMITED
} TSCheckPointStorageType;

const char* TSCheckPointStorageTypes[] = {
    "none", "unlimited", "limited", "TSCheckPointStorageType", "TSCP_STORAGE_", NULL};
...
static PetscErrorCode ts_checkpoint_create_path(TS ts, TSCheckPoint cp, PetscInt step) {
    ...
    if (cp->mode == TSCP_STORAGE_UNLIMITED) {
        fileindex = step;
    }
    if (cp->mode == TSCP_STORAGE_LIMITED) {
        fileindex = cp->n % cp->limited_size;
    }
    ...
    if (L == 0) {
        ...
        if (cp->path_prefix[L - 1] == '/') {
            PetscSNPrintf(cp->path_step, PETSC_MAX_PATH_LEN - 1, "%s%s%d", cp->path_prefix, base,
                          (int)fileindex);
        } else {
            PetscSNPrintf(cp->path_step, PETSC_MAX_PATH_LEN - 1, "%s/%s%d", cp->path_prefix, base,
                          (int)fileindex);
...
static PetscErrorCode ts_checkpoint_write(TS ts, TSCheckPoint cp) {
    ...
    PetscPrintf(PetscObjectComm((PetscObject)ts),
                "[TSCheckpoint]   writing checkpoint data for step %d -> path %s\n", (int)step,
                cp->path_step);
    PetscSNPrintf(filename, PETSC_MAX_PATH_LEN - 1, "%s/ts.bin", cp->path_step);
    ierr =
        PetscViewerBinaryOpen(PetscObjectComm((PetscObject)ts), filename, FILE_MODE_WRITE, &viewer);
    CHKERRQ(ierr);
    ierr = _TSView(ts, viewer);
    CHKERRQ(ierr);
...

    {
        ...
        if (commrank == 0) {
            ...
            PetscSNPrintf(infoname, PETSC_MAX_PATH_LEN - 1, "%s/checkpoint_info.csv",
                          cp->path_step);
...
static PetscErrorCode ts_checkpoint_load(TS ts, const char pathname[]) {
    ...
    PetscFunctionBeginUser;
    PetscPrintf(PetscObjectComm((PetscObject)ts),
                "[TSCheckpoint]   loading checkpoint data <- path %s\n", pathname);

    PetscSNPrintf(filename, PETSC_MAX_PATH_LEN - 1, "%s/ts.bin", pathname);
    ierr =
        PetscViewerBinaryOpen(PetscObjectComm((PetscObject)ts), filename, FILE_MODE_READ, &viewer);
    CHKERRQ(ierr);
    ierr = _TSLoad(ts, viewer);
    CHKERRQ(ierr);
    ierr = PetscViewerDestroy(&viewer);
    CHKERRQ(ierr);
...
PetscErrorCode ts_checkpoint(TS ts) {
    ...
    PetscFunctionBeginUser;
    ierr = PetscObjectQuery((PetscObject)ts, "_TSCheckPoint", (PetscObject*)&container);
    CHKERRQ(ierr);
    if (!container)
        SETERRQ(PetscObjectComm((PetscObject)ts), PETSC_ERR_ORDER,
                "Must call ts_checkpoint_configure() first");
    PetscContainerGetPointer(container, (void**)&tsc);
    ierr = ts_checkpoint_test(ts, tsc, &generate);
    CHKERRQ(ierr);
    if (generate) {
        /* create directory checkpoint_prefix/stepXXXX/ */
        TSGetStepNumber(ts, &step);
        ierr = ts_checkpoint_create_path(ts, tsc, step);
        CHKERRQ(ierr);

        /* write files ts.bin, state.vec, ts_impl.bin */
        ierr = ts_checkpoint_write(ts, tsc);
        CHKERRQ(ierr);
    }
    tsc->path_step[0] = '\0';
    PetscFunctionReturn(PETSC_SUCCESS);
...
PetscErrorCode ts_checkpoint_restart(TS ts, const char* tsCheckpointLoadDirectory) {
    ...
    ierr = ts_checkpoint_load(ts, load_path_prefix);
    CHKERRQ(ierr);
