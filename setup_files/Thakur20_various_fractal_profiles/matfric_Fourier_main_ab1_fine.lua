...
fractal_file = io.open ('/home/jyun/Tandem/Thakur20_various_fractal_profiles/fractal_ab_01','r')
lines = fractal_file:lines()
local fault_y = {}
local ab = {}
_y,_ab = fractal_file:read('*number', '*number')
if _y ~= nil then
...
function BP1:ab(x, y)
    local het_ab = linear_interpolation(fault_y, ab, y)
    if y > 0 then
        het_ab = self.a_b1
    end
    return het_ab
