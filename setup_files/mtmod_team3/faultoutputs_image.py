...
def fout_image(lab,outputs,dep,params,cumslip_outputs,save_dir,prefix,
    ...
    if vmax is None:
        vmax = np.max(var)
    ax = plot_image(X,Y,var,lab,outputs,cumslip_outputs,save_dir,prefix,params,zoom_frame,rths,vmin,vmax,Vths,plot_in_timestep,plot_in_sec,cb_on,save_on)
    return ax
...
def decoration(time,zoom_frame,outputs,cumslip_outputs,ver_info,Hs,acolor,rths,plot_in_timestep,plot_in_sec):
    ...
    if ver_info_opt and len(ver_info) > 0:
        if ver_info[:2] == '+ ':
            ver_info = ver_info[2:]
        # ver_info = 'Fractal stress + a-b + Dc'
...
def plot_image(X,Y,var,lab,outputs,cumslip_outputs,save_dir,prefix,params,zoom_frame,rths,vmin,vmax,Vths,plot_in_timestep,plot_in_sec,cb_on,save_on):
    ...
    if cb_on:
        ...
        # plt.colorbar(cb,extend='both',location='bottom',shrink=0.5).set_label(cb_label,fontsize=30,labelpad=15)
    ...
    fig_name = decoration(time,zoom_frame,outputs,cumslip_outputs,ver_info,Hs,acolor,rths,plot_in_timestep,plot_in_sec)
