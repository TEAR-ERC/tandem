...
- **v_th**: Velocity threshold (optional, no default). If set, output for this writer is written when the maximum slip rate VMax rises through this value, and all other adaptive output (atol/rtol/t_min/t_max) is disabled. Behaviour depends on **freq**: when **freq** is *not* set, exactly one sample is written — captured the first time VMax crosses the threshold — and the simulation is then **stopped**, giving you a single pseudo-checkpoint of the fault state at the onset of the first seismic event while minimizing the amount of output written. When **freq** is set, the run instead keeps going and keeps sampling the event (see **freq**). It is meant to have a pseudo-checkpoint for the fault state at the time of a seismic event.
- **freq**: Positive integer (optional, no default). Only has an effect when **v_th** is set. If set, output is written every **freq** monitor time steps for as long as VMax stays at or above **v_th**, instead of only once on the upward crossing, so you can capture the evolution of an event without flooding the output directory with a file per time step. Must be strictly positive (0 or negative values are rejected when the parameter file is parsed). If not set, only a single upward-crossing sample is written and the simulation is aborted immediately afterwards (see **v_th**). With **freq** set, output stops once VMax drops back below **v_th**, and each excursion above the threshold (an "event") is written into its own ``event_N`` subfolder next to the writer's output directory (``event_0`` for the first excursion, ``event_1`` for the second, and so on), with the usual step numbering restarting at 0 inside each folder.
