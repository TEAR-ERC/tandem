...
# sample command line usage: $0 clang-format(version == 19.1.2) $TANDEM_SOURCE_DIR
...
format() {
    ...
    #When installed with anaconda, the address of the repository and commit hash may be displayed by clang-format -v
    #clang-format version 19.1.2 (https://github.com/conda-forge/clangdev-feedstock 557de63f17dd74f6daaa840e9730b2b67f475df2)

    local formatter_version=$(${formatter} --version | sed 's/ (https.*//')
    if [ "${formatter_version}" != "clang-format version 19.1.2" ]; then
        echo "Your clang-format tool in \"${formatter}\" does not have the correct version (should be 19.1.2). Given: ${formatter_version}"
        echo "Hint: you may install the required clang-format via pip, by typing: pip3 install clang-format==19.1.2"
        exit 176
    fi
