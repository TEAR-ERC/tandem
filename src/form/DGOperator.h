...
public:
    ...
    void rhs(BlockVector& vector, long int sourceTag) {
    auto bs = lop_->block_size();
    ...
    // Volume RHS contributions
    if constexpr (std::experimental::is_detected_v<rhs_volume_t, LocalOperator>) {
        for (std::size_t elNo = 0;
             elNo < topo_->numLocalElements();
             ++elNo) {

            scratch_.reset();

            auto B0 =
                access_handle.subtensor(slice{}, elNo);
    ...
    // Facet RHS contributions
    if constexpr (
        std::experimental::is_detected_v<rhs_skeleton_t, LocalOperator> ||
        std::experimental::is_detected_v<rhs_boundary_t, LocalOperator>) {

        for (std::size_t fctNo = 0;
             fctNo < topo_->numLocalFacets();
             ++fctNo) {
            ...
            // Only assemble facets belonging to this source tag
            if (info.facetTag != sourceTag) {
                continue;
            }
            ...
            if (info.up[0] != info.up[1]) {
                ...
                auto B0 =
                    info.inside[0]
                        ? access_handle.subtensor(slice{}, ib0)
                        : sv(a_scratch);

                auto B1 =
                    info.inside[1]
                        ? access_handle.subtensor(slice{}, ib1)
                        : sv(a_scratch);
    ...
    // Post-skeleton volume contribution
    if constexpr (
        std::experimental::is_detected_v<rhs_volume_post_skeleton_t, LocalOperator>) {
