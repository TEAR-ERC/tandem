#!/usr/bin/env python3
'''
Automatically write scripts using matched time of perturbation better
By Jeena Yun
...
fcoeff = 0.4
dt = 0.01
target_eventid = 31        # Between 0 to 63
...
matched_save_dir = '/export/dump/jyun/%s/match%d'%(model_n,target_eventid)
# matched_save_dir = '/import/freenas-m-05-seissol/jyun/%s/match%d'%(model_n,target_eventid)
...
print('Event %d; System-size Event Index %d; Hypocenter Depth: %1.2f [km]'%(idx,target_eventid,evdep[idx]))
...
if write_on:
    ...
    if scenario_name in scenarios: # Check if the scenario is already there
        ...
        flua.write('\n%s = pert.new{model_n=\'%s\',strike=%d,fcoeff=%1.1f,dt=%1.2f,init_time=%1.18e}'%(scenario_name,seissol_model_n,strike,fcoeff,dt,init_time))
        flua.close()
    ...
    fshell.write('mpiexec -bind-to core -n %d tandem $setup_dir/%s --petsc -ts_checkpoint_load ../match%d/outputs/checkpoint/step%d '
                ...
                %(n_node,fname_toml.split('/')[-1],target_eventid,stepnum,dt,maxstep,ckp_freq_ptime,ckp_freq_cputime))
    fshell.close()
    ...
    fshell.write('# Generate output and change directory name\n')
    fshell.write('model_n=%s\n'%(model_n))
    fshell.write('pert_branch_n=%s\n'%(run_branch_n))
    fshell.write('save_dir=/export/dump/jyun/$model_n/$pert_branch_n\n')
    fshell.write('echo $save_dir\n')
    fshell.write('mkdir -p $save_dir\n')
    fshell.write('mv /export/dump/jyun/$model_n/outputs_$pert_branch_n $save_dir\n')
    fshell.write('mv $save_dir/outputs_$pert_branch_n $save_dir/outputs\n')
    fshell.write('python get_plots.py $save_dir -c\n\n')

    fshell.write('# Generate checkpoint time info\n')
    fshell.write('./read_time_recursive /export/dump/jyun/$model_n/$pert_branch_n\n\n')
